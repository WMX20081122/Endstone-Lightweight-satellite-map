...
3. 安装 unmined-cli Linux x64 到 `plugins/bdslm/unmined-cli/`
4. 重启服务器
...
| `webserver.bindAddress` | 绑定地址（建议 127.0.0.1） | "127.0.0.1" |
...
| `paths.unminedCli` | unmined-cli 路径 | "./plugins/bdslm/unmined-cli/unmined-cli" |
...
| 命令 | 说明 | 权限 |
|------|------|------|
| `/bdslm render [dimension]` | 手动触发渲染 | bdslm.command |
| `/bdslm status` | 查看插件状态 | bdslm.command |
| `/bdslm reload` | 重载配置和标记 | bdslm.command |
| `/bdslm marker add <name> <x> <z>` | 添加标记点 | bdslm.command |
| `/bdslm marker remove <name>` | 移除标记点 | bdslm.command |
| `/bdslm marker list` | 列出所有标记 | bdslm.command |
...
```
plugins/
├── endstone_bdslm-1.0.2-py3-none-any.whl   # 插件包
└── bdslm/                                    # 数据目录（自动创建）
    ├── config.json                           # 配置
    ├── markers.json                          # 标记
    ├── players.json                          # 玩家位置（自动更新）
    ├── unmined-cli/                          # unmined-cli 程序
    │   └── unmined-cli
...
- ✅ 默认绑定 127.0.0.1，避免端口问题
- ✅ 配置/标记格式与 bdslm-js 完全兼容

## 📝 版本历史

### v1.0.2
- 修复 `run_task_timer` → `run_task(plugin, task, delay, period)` (Endstone 0.11.4 API)
- 修复 `register_event` → `register_events` + `@event_handler` 装饰器
- 修复 `subprocess.run(text=True)` ASCII 编码崩溃
- 修复 `bindAddress` 默认值

### v1.0.0
- 初始版本，完全复刻 bdslm-js 功能
